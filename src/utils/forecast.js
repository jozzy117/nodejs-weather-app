...
const forecast = (lon, lat, callback)=>{
    ...
    .then(({body})=>{
        callback( body.current.weather[0].description + '! The current temperature is ' + body.current.temp + ' degree, with the daily temperature ranging from '+ body.daily[0].temp.min + ' - ' + body.daily[0].temp.max +' degree. There is a ' + body.daily[0].pop + ' percent chance of rain.', undefined)
    })
    .catch((error)=>{
        if (error.response){
